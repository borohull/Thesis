...
\begin{table}[H]
    ...
    \begin{tabular}{|l|l|p{0.42\textwidth}|}
        \hline
        \textbf{Endpoint} & \textbf{Test} & \textbf{Assertion} \\
        \hline \hline
        \texttt{GET /health}   & server up         & \texttt{status == "ok"}, \texttt{ollama} key present \\
        \hline
        \texttt{GET /health}   & Ollama online     & \texttt{ollama == true} \\
        \hline
        \texttt{POST /chat}    & basic response    & 200, non-empty answer, sources list, \texttt{response\_ms > 0} \\
        \hline
        \texttt{POST /chat}    & in-scope query    & answer length $> 20$ characters \\
        \hline
        \texttt{POST /chat}    & out-of-scope query & 200, non-empty answer \\
        \hline
        \texttt{POST /chat}    & source keys       & each source contains \texttt{chunk\_id} and \texttt{file} \\
        \hline
        \texttt{POST /chat}    & session logging   & session appears in \texttt{GET /sessions} after chat \\
        \hline
        \texttt{GET /sessions} & session created   & session ID present after chat with \texttt{session\_id} \\
        \hline
        \texttt{GET /sessions/\{id\}/messages} & messages retrievable & one message with correct \texttt{user\_message} field \\
        \hline
        \texttt{DELETE /sessions/\{id\}} & session delete & 204, session absent from list afterwards \\
        \hline
        \texttt{POST /upload}  & valid PDF         & 200, \texttt{status} is \texttt{"indexed"} or \texttt{"already\_indexed"} \\
        \hline
        \texttt{POST /upload}  & duplicate file    & \texttt{status == "already\_indexed"} on second upload \\
        \hline
        \texttt{POST /upload}  & invalid extension & 415 Unsupported Media Type \\
        \hline
        \texttt{GET /info}     & collection size   & \texttt{total\_chunks} is a non-negative integer \\
        \hline
        \texttt{GET /info}     & embedding model   & \texttt{embedding\_model == "all-MiniLM-L6-v2"} \\
        \hline
        \texttt{GET /models}   & model list        & \texttt{models} is a non-empty list, \texttt{default} key present \\
        \hline
    \end{tabular}
...
Because LLM output is non-deterministic, no test asserts exact answer text; all assertions target response structure, status codes, and field types.
...
python -m pytest tests/e2e/ -m e2e -v --no-cov
\end{lstlisting}
