...
This chapter describes the full implementation of the ELTE IK Assistant, a locally deployed chatbot for the Faculty of Informatics. The system is built in four major layers: data collection and ingestion, document embedding and vector storage, a Retrieval-Augmented Generation (RAG) backend, and a web-based frontend. Each section below covers one of these layers in detail, including the key design choices, data structures, and code that make the system work.
...
% ─────────────────────────────────────────────────────────────
\section{Data Collection and Ingestion}
\label{sec:impl-data}
...
\subsection{Incremental Ingestion with SHA-256 Manifest}
...
The chosen model is \texttt{all-MiniLM-L6-v2}, a compact sentence transformer from the Sentence-Transformers library. It produces 384-dimensional vectors and runs efficiently on CPU, which is important for local deployment without a dedicated GPU. Although larger embedding models exist, this model offers a very good accuracy-to-resource trade-off for English factual retrieval tasks.
...
\subsection{Local LLM via Ollama}
...
\subsection{End-to-End Query Function}
