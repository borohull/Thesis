...
\begin{enumerate}
    \item Install \textbf{Ollama} from \texttt{ollama.com} and pull the model:
\begin{lstlisting}[language=bash]
ollama pull llama3.2:3b
\end{lstlisting}

    \item Create and activate a Python virtual environment, then install dependencies:
\begin{lstlisting}[language=bash]
python -m venv .venv
...
\end{lstlisting}
    ...
    \item Start the API server from the repository root:
\begin{lstlisting}[language=bash]
uvicorn app.main:app --reload
\end{lstlisting}

    \item Open the chat interface at \texttt{http://localhost:8000/static/index.html}.
\end{enumerate}

\subsection*{Configuration}

All tunable parameters are defined in \texttt{app/settings.py}.
The most commonly adjusted values are listed in Table~\ref{tab:settings}.
